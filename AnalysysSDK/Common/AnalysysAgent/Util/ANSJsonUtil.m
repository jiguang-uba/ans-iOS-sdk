...
+ (NSArray *)convertToArrayWithString:(NSString *)jsonStr {
    NSError *error = nil;
    NSData *getJsonData = [jsonStr dataUsingEncoding:NSUTF8StringEncoding];
    if (!getJsonData) {
        return nil;
    }
    NSArray *arr = [NSJSONSerialization JSONObjectWithData:getJsonData options:NSJSONReadingMutableContainers error:&error];
    if (error == nil && [arr isKindOfClass:[NSArray class]]) {
        return arr;
    }
    return nil;
}
