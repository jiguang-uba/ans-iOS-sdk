...
+ (NSData*)gzipData:(NSData*)pUncompressedData {
    if (!pUncompressedData || [pUncompressedData length] == 0) {
        ANSDebug(@"%s: Error: Can't compress an empty or null NSData object.", __func__);
        return nil;
    }
    ...
    if (initError != Z_OK)
    {
        ...
        ANSDebug(@"%s: deflateInit2() Error: \"%@\" Message: \"%s\"", __func__, errorMsg, zlibStreamStruct.msg);
        return nil;
    }
    ...
    if (deflateStatus != Z_STREAM_END)
    {
        ...
        ANSDebug(@"%s: zlib error while attempting compression: \"%@\" Message: \"%s\"", __func__, errorMsg, zlibStreamStruct.msg);
