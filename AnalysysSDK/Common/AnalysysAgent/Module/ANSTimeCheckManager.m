...
- (void)requestWithServer:(NSString *)serverUrl block:(void(^)(NSTimeInterval serverTimerDiff))block {
    if (!AnalysysConfig.allowTimeCheck) {
        return;
    }
    ...
    ANSNetworking *network = [[ANSNetworking alloc] initWithURL:[NSURL URLWithString:serverUrl]];
    network.securityPolicy = AnalysysConfig.securityPolicy;
    
    [network getRequestWithParameters:nil success:^(NSURLResponse *response, NSData *responseData) {
        ...
        // 2020-01-01
        if (serverTimeInterval < 1577808000) {
            self->_serverTimerDiff = 0;
        } else {
