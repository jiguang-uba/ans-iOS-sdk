...
+ (void)setAnalysysAgentHybrid:(WKWebViewConfiguration *)config {
    ...
    @try {
        ...
        [config.userContentController addScriptMessageHandler:[ANSWKScriptMessageHandler shareInstance] name:@"AnalysysAgent"];
...
+ (void)setAnalysysAgentHybridScriptMessage:(WKScriptMessage *)scriptMessage {
    ...
    @try {
        if ([scriptMessage.name isEqualToString:@"AnalysysAgent"]) {
            NSDictionary *dic = scriptMessage.body;
            if (!dic) {
                return;
            }
            NSLog(@"body : %@",scriptMessage.body);
            ...
            if ([hybridInstance respondsToSelector:selector]) {
                if (funcCallBack.length == 0) {
                    ...
                    ((void (*)(id, SEL, NSMutableArray*, id))[hybridInstance methodForSelector:selector])(hybridInstance, selector, desArray, web);
                }
            } else {
...
+ (void)resetAnalysysAgentHybrid:(WKWebViewConfiguration *)config {
    [config.userContentController removeScriptMessageHandlerForName:@"AnalysysAgent"];
}
...
- (void)pageView:(NSArray *)param {
    ...
    if (param.count == 2) {
        ...
        if ([properties isKindOfClass:[NSDictionary class]]) {
            NSDictionary *pageProperties = [NSMutableDictionary dictionaryWithDictionary:properties];
            if (![pageProperties.allKeys containsObject:ANSPageReferrerUrl]) {
                //  添加来自原生页面标识
                [pageProperties setValue:[ANSPageAutoTrack shareInstance].referrerPageUrl forKey:ANSPageReferrerUrl];
            }
            [AnalysysAgent pageView:pageId properties:pageProperties];
        } else {
...
- (void)registerSuperProperty:(NSArray *)param {
    ...
    if (superPropertyName && superPropertyValue) {
        ...
        [ANSHybrid registerHybirdSuperProperties:dic];
    }
}
...
- (void)registerSuperProperties:(NSArray *)param {
    ...
    if ([properties isKindOfClass:[NSDictionary class]]) {
        [ANSHybrid registerHybirdSuperProperties:properties];
    } else {
        ANSBriefWarning(@"Hybrid: registerSuperProperty parameter must be {key:value}.");
    }
}
...
- (void)unRegisterSuperProperty:(NSArray *)param {
    ...
    if ([superPropertyName isKindOfClass:[NSString class]]) {
        [ANSHybrid unRegisterHybirdSuperProperty:superPropertyName];
    } else {
...
- (void)clearSuperProperties:(NSArray *)param {
    [ANSHybrid clearHybirdSuperProperties];
}
