...
- (void)handleException:(NSException *)exception successBlock:(void(^)(void))ret{
    ...
    [[AnalysysSDK sharedManager] trackCrashInfo:@{@"$crash_data":crashDataString ?: @""}];
...
void ANSHandleException(NSException *exception)
{
    ...
    NSException *tmpException = [NSException exceptionWithName:[exception name] reason:[exception reason] userInfo:userInfo];
    
    [[[ANSUncaughtExceptionHandler alloc] init] handleException:tmpException successBlock:^{
