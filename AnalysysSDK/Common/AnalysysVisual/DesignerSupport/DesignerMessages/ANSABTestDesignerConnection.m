...
- (void)sendMessage:(id<ANSABTestDesignerMessage>)message {
    if (_connected) {
        ANSVisualBriefLog(@"Send ws command: %@", message.type);
...
- (void)sendJsonMessage:(id)object {
    ...
    if (parseError) {
        ANSVisualBriefError(@"Echo json error:%@",parseError);
        return;
    }
...
- (void)webSocketDidOpen:(ANSWebSocket *)webSocket {
    ANSVisualBriefLog(@"WS connect success. url: %@", webSocket.url);
    _commandQueue.suspended = NO;
//    [self showConnectedViewWithLoading:YES];
}

- (void)webSocket:(ANSWebSocket *)webSocket didFailWithError:(NSError *)error {
    ANSVisualBriefLog(@"ws connect failed. url: %@ error:  %@", webSocket.url, error);
