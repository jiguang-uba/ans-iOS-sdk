...
+ (void)findBindView:(UIView *)bindView withData:(NSArray *)bindData bindProps:(void (^)(NSString*event_id, NSDictionary *props))bindProps {
    ...
    [bindData enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        NSString *currentPage = NSStringFromClass([ANSControllerUtils currentViewController].class);
        
        //如果是RN的UIController就进行使用最新的url的pageName
        UIViewController *currentCon = [ANSControllerUtils currentViewController];
        if (currentCon) {
            UIView *uiview = currentCon.view;
            if(uiview) {
                
                if ([uiview isKindOfClass:NSClassFromString(@"RCTRootView")]) {
                    NSString *rnPageName = [ANSPageAutoTrack shareInstance].rnPageName;
                    currentPage = rnPageName;
                }
            }
            
        }
        
        if (bind_page && ![bind_page containsObject:currentPage]) {
            return;
        }
        ...
        //按newPath查找对应的绑定控件
        if (newPath) {
            NSArray *arrtmp = tmp_ans_analysysViewPath;
            NSMutableArray* tmpArray= [NSMutableArray arrayWithArray:tmp_ans_analysysViewPath];
            BOOL flag = [self findRNScroll:tmpArray];
            
            if (flag) {
                tmp_ans_analysysViewPath = tmpArray;
                
                tmpArray= [NSMutableArray arrayWithArray:newPath];
                [self findRNScroll:tmpArray];
                newPath = tmpArray;
            }
            
            if (![self compare_path:tmp_ans_analysysViewPath dataCachePath:newPath]) {
                tmp_ans_analysysViewPath = arrtmp;
                return;
            }
            tmp_ans_analysysViewPath = arrtmp;
...
+ (BOOL)findRNScroll:(NSMutableArray *)arr1 {
    ...
    for (int i = 0 ; i < count;i++){
        ...
        NSString *str = [obj objectForKey:@"viewType"];
        if([str isEqualToString:@"RNCSafeAreaProvider"]) {
            find = i;
        }
        
        if([str isEqualToString:@"RCTScrollView"]) {
            flag = TRUE;
        }
    }
    
    if(flag&&find>0) {
        NSUInteger tmpCount = find + 2;
        if(tmpCount<count) {
            [arr1 removeObjectAtIndex:(find+2)];
            flagBack = TRUE;
        }
    }
    return flagBack;
            ...
            if (associated_newPath && h5_path) {
                [ANSVisualSearch findAssociatedWithViewPath:associated_newPath step:associated_step withOriginalView:bindView successBlock:^(UIView * _Nonnull associatedView) {
                    ...
                    [((WKWebView *)associatedView) evaluateJavaScript:jsStr completionHandler:^(NSString *props, NSError * _Nullable error) {
                        if (error) {
                            ANSDebug(@"WebView: getProperty error");
                        } else {
                ...
                ANSDebug(@"WebView: new_pat/h5_path error");
            }
        }];
    }
}
    ...
    if ([rootView isKindOfClass:[UIWindow class]]) {
        ...
        [rootView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:NSClassFromString(@"UITableViewWrapperView")]) {
                [rootviews addObjectsFromArray:obj.subviews];
            } else {
...
* case2:以iOS9系统在UITableView下新增了UITableViewWrapperView 为了统一new_path由 子->父 需要将该节点去掉
*/
+ (UIView *)bridge_subviewTosuperview:(UIView *)view {
    ...
    } else if ([NSStringFromClass([view class]) isEqualToString:@"UITableViewWrapperView"]) {
