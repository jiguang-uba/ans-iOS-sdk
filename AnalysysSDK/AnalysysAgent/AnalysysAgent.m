...
+ (void)setAnalysysAgentHybrid:(WKWebViewConfiguration *)config scriptMessageHandler:(id)handler {
    [[AnalysysSDK sharedManager] setAnalysysAgentHybrid:config scriptMessageHandler:handler];
    [[AnalysysSDK sharedManager] setWebViewVisualConfig:config scriptMessageHandler:handler];
}

+ (void)setAnalysysAgentHybridScriptMessage:(WKScriptMessage *)scriptMessage {
    [[AnalysysSDK sharedManager] setAnalysysAgentHybridScriptMessage:scriptMessage];
    [[AnalysysSDK sharedManager] setScriptMessage:scriptMessage];
}

+ (void)resetAnalysysAgentHybrid:(WKWebViewConfiguration *)config {
    [[AnalysysSDK sharedManager] resetAnalysysAgentHybrid:config];
    [[AnalysysSDK sharedManager] resetWebViewVisualConfig:config];
}
...
+ (AnalysysSSManagerAAABBB *)getSSManager {
    return [AnalysysSSManagerAAABBB sharedManager];
}

@end


@implementation AnalysysAgent (XMGJYH)

+ (void)registerHybirdSuperProperties:(NSDictionary *)superProperties {
    [ANSHybrid registerHybirdSuperProperties:superProperties];
}
...
+ (void)analysysAgentGetEventList:(WKWebView *)web {
    [[AnalysysSDK sharedManager] analysysAgentGetEventList:web];
}

+ (void)analysysAgentGetProperty:(WKWebView *)web withParams:(NSArray *)params {
    [[AnalysysSDK sharedManager] analysysAgentGetProperty:web withParams:params];
}

+ (void)analysysAgentTrack:(WKWebView *)web withParams:(NSArray *)params {
    [[AnalysysSDK sharedManager] analysysAgentTrack:web withParams:params];
}
