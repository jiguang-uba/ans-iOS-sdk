...
- (void)setScreenshot:(UIImage *)screenshot {
    ...
    [self setPayloadObject:@(true) forKey:@"new_feature"];
}

- (void)addExtroResponseInfo {
    @try {
        //  与热图模块页面标识同步
        __block NSString *className = NSStringFromClass([ANSControllerUtils currentViewController].class);
            ...
            if (currentCon) {
                ...
                if (uiview) {
                    if ([uiview isKindOfClass:NSClassFromString(@"RCTRootView")]) {
                        className = [ANSPageAutoTrack shareInstance].rnPageName;
                    }
                }
            }
