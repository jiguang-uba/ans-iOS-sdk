...
+ (void)autoTrack {
    [NSThread ansRunOnMainThread:^{
        void (^viewDidAppearBlock)(id, SEL, id) = ^(id obj, SEL sel, NSNumber *num) {
            //自动pv重置sessionID，在前面避免后面sessionId不统一
            [[ANSSessionManager sharedManager] pvSession];
            
            UIViewController *controller = (UIViewController *)obj;
            
            [[PageViewDuration sharedInstance] ans_ViewDidAppear:controller];
