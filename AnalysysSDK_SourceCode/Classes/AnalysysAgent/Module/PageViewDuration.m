...
- (void)ans_ViewDidAppear:(UIViewController *)controller {
    ...
    @try {
        ...
        if (currentViewController) {
            ...
            NSString *url = NSStringFromClass([controller class]);

            if ([[AnalysysSDK sharedManager] isTrackWithCloseClassName:url]) {
                //如果在黑名单中设置该类名，currentController不发生变化便不会触发pageclose，不能直接放在sendpageclose方法中
                return;
            }
