...
- (void)trackHeatMap:(UITouch *)touch {
    ...
    //同RN匹配
    NSString *str = NSStringFromClass([_touchView class]);
    if([str isEqualToString:@"RCTTextView"])  {
        id tmpTouch = [_touchView nextResponder];
        
        if(tmpTouch){
            tmpTouch = [tmpTouch nextResponder];
            if(tmpTouch){
                _touchView = tmpTouch;
            }
        }
