...
- (void)packageDataWithView:(UIView *)view {
    ...
    [ANSAllBuryPoint trackAllBuryPoint:model];
}
...
+ (void)allbury_viewClicked:(NSNumber * _Nonnull)view_id {
    if ([[self sharedManager] autoTrack] == YES) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if ([mager respondsToSelector:viewWithReactTag]) {
                ...
                if (view) {
                    ANSDebug(@"ReactNativeView : %@",view);
                    if ([view isKindOfClass:NSClassFromString(@"RNCSlider")] ||
                        [view isKindOfClass:NSClassFromString(@"RCTView")]) {
                        UIViewController *vc = [ANSControllerUtils findViewControllerByView:view];
                        if ([[self sharedManager] checkIsReport:view withTargat:vc]) {
                            [[self sharedManager] packageDataWithSender:view event:nil];
                        }
                    }
                }
            }
...
- (void)packageDataWithSender:(id)sender event:(UIEvent *)event {
    if ([sender isKindOfClass:[UISwitch class]] ||
        ...
        [sender isKindOfClass:NSClassFromString(@"RCTView")] ||
        [sender isKindOfClass:NSClassFromString(@"RNCSlider")] ) {
...
+ (void)trackAllBuryPoint:(ANSAllBuryPointModel *)model {
    [ANSQueue dispatchAsyncLogSerialQueueWithBlock:^{
        //  数据组装必须放入线程队列中，防止alias事件中$is_login部分字段错误
        [[AnalysysSDK sharedManager] trackAllWithProperties:[model toDictionary]];
    }];
}
...
- (void)setAutoClickBlackListByViewTypes:(NSSet<NSString *> *)viewNames {
    if (![viewNames isKindOfClass:NSSet.class] || viewNames.count == 0) {
        return;
    }
