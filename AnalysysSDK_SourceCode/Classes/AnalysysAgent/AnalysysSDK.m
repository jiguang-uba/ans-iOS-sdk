...
@property (nonatomic, strong) ANSNetworking *networking;
@property (atomic, strong) volatile NSDictionary *commonProperties;  // 自定义常用属性
@property (atomic, strong) volatile NSDictionary *superProperties;   // 用户通用属性
@property (atomic, copy) volatile NSString *userId;  //  当前用户
...
@implementation AnalysysSDK {
    ...
    NSMutableSet *_pageCloseBalckList;// 忽略部分页面pageclose采集
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _pageCloseBalckList = [NSMutableSet set];
        ...
        [[ANSSessionManager sharedManager] monitorSession];
    ...
    @try {
        ...
        dispatch_once(&autoTrackOnceToken, ^{
            ...
            if(config.autoPageViewDuration) {
                [[PageViewDuration sharedInstance] start];
            }
        });
        ...
        if (config.baseUrl.length > 0) {
            ...
            self.networking = [[ANSNetworking alloc] initWithURL:[NSURL URLWithString:upServerUrl]];
            self.networking.securityPolicy = config.securityPolicy;
...
- (void)setObserverListener:(id)observerListener {
    ...
    // EA xwho 回传
    if (self.delegate && [self.delegate respondsToSelector:@selector(onUserProfile:)]) {
        [self.delegate onUserProfile:@{@"xwho": userId ?: @""}];
    }
}
...
- (void)setUploadURL:(NSString *)uploadURL {
    ...
    self.networking = [[ANSNetworking alloc] initWithURL:[NSURL URLWithString:serverUrl]];
    self.networking.securityPolicy = AnalysysConfig.securityPolicy;
    
    [[ANSTimeCheckManager shared] requestWithServer:serverUrl block:^(NSTimeInterval serverTimerDiff) {
        [self uploadDataType:@""];
        // EA xwho 回传
        if (self.delegate && [self.delegate respondsToSelector:@selector(onUserProfile:)]) {
            [self.delegate onUserProfile:@{@"calibration_time": [NSNumber numberWithDouble:serverTimerDiff]}];
        }
    }];
...
- (void)track:(NSString *)event properties:(NSDictionary *)properties {
    ...
    if (![event isKindOfClass:NSString.class]) {
        event = nil;
        ANSBriefWarning(@"event_id is not <NSString>.");
    } else if ([event isKindOfClass:NSString.class] && event.length == 0) {
        event = nil;
        ANSBriefWarning(@"event_id is empty.");
    }
...
- (void)setPageViewWhiteListByPages:(NSSet<NSString *> *)controllers {
    if (![controllers isKindOfClass:NSSet.class] || controllers.count == 0) {
        return;
    }
...
- (NSMutableSet *)getBlackList
{
    ...
    ANSPropertyLock();
    result = [_pageViewBlackList copy];
    ANSPropertyUnlock();
    return result;
}

- (NSMutableSet *)getPageCloseBlackList
{
    NSMutableSet *result = nil;
    ANSPropertyLock();
    result = [_pageCloseBalckList copy];
    ANSPropertyUnlock();
    return result;
}
...
- (void)setPageCloseBlackList:(NSSet<NSString *> *)controllers
{
    ...
    NSSet *sControllers = [controllers mutableCopy];
    ANSPropertyLock();
    [_pageCloseBalckList setSet:sControllers];
    ANSPropertyUnlock();
}
...
- (void)setHeatmapIgnoreAutoClickByPage:(NSSet<NSString *> *)controllerNames {
    if (![controllerNames isKindOfClass:NSSet.class] || controllerNames.count == 0) {
        return;
    }
...
//        ANSPropertyLock();
        ...
        NSMutableDictionary *tmp = [NSMutableDictionary dictionaryWithDictionary:[self.superProperties copy]];
        [tmp addEntriesFromDictionary:superProperties];
        _superProperties = [NSDictionary dictionaryWithDictionary:tmp];
        
        BOOL result = [ANSFileManager archiveSuperProperties:[self.superProperties copy]];
//        ANSPropertyUnlock();
        if (result) {
            ANSDataCheckLog *checkResult = [[ANSDataCheckLog alloc] init];
            checkResult.resultType = AnalysysResultSetSuccess;
...
- (NSDictionary *)getSuperPropertiesValue {
    ...
    [superProperties addEntriesFromDictionary:[_superProperties copy]];

    return [superProperties copy];
}

/** 同步接口会阻塞只对外使用：获取hybird通用属性，取App与js集合，且已App为准 */
- (NSDictionary *)getSuperPropertiesValueSync {
    __block NSDictionary *tmp = nil;
    dispatch_semaphore_t sema =  dispatch_semaphore_create(0);

    dispatch_block_t block = ^(){
        NSDictionary *hybirdSuperProperty = [ANSFileManager unarchiveHybridSuperProperties];
        NSMutableDictionary *superProperties = [NSMutableDictionary dictionaryWithDictionary:hybirdSuperProperty];
        [superProperties addEntriesFromDictionary:[self.superProperties copy]];

        tmp =  [superProperties copy];
...
/** 同步接口会阻塞只对外使用：获取hybird某个通用属性，取App与js合集，若key相同则以App为准，供外部使用
 */
- (id)getSuperPropertySync:(NSString *)superPropertyName {
    __block id retValue = nil;
    dispatch_semaphore_t sema =  dispatch_semaphore_create(0);

    dispatch_block_t block = ^(){
        NSDictionary *superProperties = [self getSuperPropertiesValue];
        retValue = superProperties[superPropertyName];
        dispatch_semaphore_signal(sema);
    };
    [ANSQueue dispatchAsyncLogSerialQueueWithBlock:block];
    dispatch_semaphore_wait(sema, DISPATCH_TIME_FOREVER);
...
/** 获取hybird某个通用属性，取App与js合集，若key相同则以App为准，供外部使用
 */
...
- (NSDictionary *)getPresetProperties {
    ...
    if (_isSDKInit) {
        NSString *session = [[ANSSessionManager sharedManager] sessionId];
...
- (void)identify:(NSString *)anonymousId {
    ...
    ANSPropertyLock();
    NSMutableDictionary *tmpCommonProperties = [NSMutableDictionary dictionaryWithDictionary:[_commonProperties copy]];
    ...
    BOOL result = [ANSFileManager archiveCommonProperties:[_commonProperties copy]];
    ANSPropertyUnlock();
...
- (void)alias:(NSString *)aliasId originalId:(NSString *)originalId {
    ...
    dispatch_block_t block = ^(){
        ...
        ANSPropertyLock();
        NSMutableDictionary *tmpCommonProperties = [NSMutableDictionary dictionaryWithDictionary:[self->_commonProperties copy]];
        ...
        [ANSFileManager archiveCommonProperties:[self->_commonProperties copy]];
        ...
        if (originalId.length > 0) {
            ...
            NSString *distictId = self.commonProperties[ANSUUID];
            if (!distictId) {
                distictId = [[NSUUID UUID] UUIDString];

                NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:[self.commonProperties copy]];
                dic[ANSUUID] = distictId;
                self.commonProperties = [NSDictionary dictionaryWithDictionary:dic];
                [ANSFileManager archiveCommonProperties:[self.commonProperties copy]];
            }
    ...
    // EA xwho 回传
    if (self.delegate && [self.delegate respondsToSelector:@selector(onUserProfile:)]) {
        [self.delegate onUserProfile:@{@"xwho": aliasId ?: @""}];
    }
}
...
- (void)setAnalysysAgentHybrid:(WKWebViewConfiguration *)config{
    [ANSHybrid setAnalysysAgentHybrid:config];
}

- (void)resetAnalysysAgentHybrid:(WKWebViewConfiguration *)config {
    [ANSHybrid resetAnalysysAgentHybrid:config];
}

- (void)analysysAgentGetEventList:(WKWebView *)web {
    if (![self isSDKInitBeforeInterface:@"analysysAgentGetEventList:"]) {
        return;
    }
    [ANSModuleProcessing analysysAgentGetEventList:web];
}

- (void)analysysAgentGetProperty:(WKWebView *)web withParams:(NSArray *)params {
    if (![self isSDKInitBeforeInterface:@"analysysAgentGetProperty:withParams:"]) {
        return;
    }
    [ANSModuleProcessing analysysAgentGetProperty:web withParams:params];
}

- (void)analysysAgentTrack:(WKWebView *)web withParams:(NSArray *)params {
    if (![self isSDKInitBeforeInterface:@"analysysAgentTrack:withParams:"]) {
        return;
    }
    [ANSModuleProcessing analysysAgentTrack:web withParams:params];
}
...
- (void)setWebViewVisualConfig:(WKWebViewConfiguration *)config{
    if (![self isSDKInitBeforeInterface:@"setWebViewVisualConfig:scriptMessageHandler:"]) {
        return;
    }
    [ANSModuleProcessing setWebViewVisualConfig:config scriptMessageHandler:[ANSWKScriptMessageHandler shareInstance]];
}

/** 对WKWebView进行js注入，方法注销 */
- (void)resetWebViewVisualConfig:(WKWebViewConfiguration *)config {
    if (![self isSDKInitBeforeInterface:@"resetWebViewVisualConfig:"]) {
        return;
    }
    [ANSModuleProcessing resetWebViewVisualConfig:config];
}

/** 对js回调方法进行监听 */
- (void)setScriptMessage:(WKScriptMessage *)scriptMessage {
    if (![self isSDKInitBeforeInterface:@"setScriptMessage:"]) {
        return;
    }
    [ANSModuleProcessing setScriptMessage:scriptMessage];
}
...
- (void)autoPageViewWithProperties:(NSDictionary *)properties {
    //  已在队列中
...
- (void)trackAllWithProperties:(NSDictionary *)properties {
    //  已在队列中
    [self saveUploadInfo:properties event:ANSUserClick handler:^{}];
}

- (void)trackCrashInfo:(NSDictionary *)info {
    NSDictionary *crashEvent = [ANSDataProcessing processAppCrashProperties:info];
    [self saveUploadInfo:crashEvent event:ANSEventCrash handler:^{}];
}
...
- (void)applicationWillResignActiveNotification:(NSNotification *)notification {
    ...
    [[PageViewDuration sharedInstance] applicationWillResignActiveNotification:notification];
    
    [ANSQueue dispatchAsyncLogSerialQueueWithBlock:^{
        NSDictionary *endEvent = [ANSDataProcessing processAppEnd];
        [self saveUploadInfo:endEvent event:ANSEventAppEnd handler:^{}];
        
        [[ANSSessionManager sharedManager] updateEnd];
    }];
}
...
- (void)trackAppStartEvent {
    dispatch_block_t block = ^(){
        ...
        //必须在反射组装之前
        [[ANSSessionManager sharedManager]startUpSession];
        
        NSDictionary *startEvent = [ANSDataProcessing processAppStartProperties:nil];
        if (!startEvent) {
            ANSDataCheckLog *checkResult = [[ANSDataCheckLog alloc] init];
...
- (void)profileResetWithType:(ANSResetType)resetType {
    ANSPropertyLock();
    NSMutableDictionary *tmp = [NSMutableDictionary dictionaryWithDictionary:[_commonProperties copy]];
    ...
    [ANSFileManager archiveSuperProperties:[_superProperties copy]];
    _commonProperties = tmp;
    [ANSFileManager archiveCommonProperties:[_commonProperties copy]];
    ANSPropertyUnlock();
    
    [self updateUserId];
    
    [[ANSSessionManager sharedManager] resetSession];
...
- (void)flushDataIfIgnorePolicy:(BOOL)ignoreDelay {
    ...
    BOOL (^uploadBlock)(NSString*, NSDictionary*, NSString*) = ^(NSString *serverUrl, NSDictionary *httpHeader, NSString *uploadInfo) {
        @try {
            ...
            if (!self.networking) {
                ANSBriefWarning(@"postRequestWithHeader-->networking is null");
                return uploadStatus;
            }
            [self.networking postRequestWithHeader:httpHeader
                                              body:uploadInfo
...
- (BOOL)isTrackWithCloseClassName:(NSString *)className {
    ANSPropertyLock();
    BOOL retValue = [_pageCloseBalckList containsObject:className];
    ANSPropertyUnlock();
    return retValue;
}
...
- (BOOL)isSDKInitBeforeInterface:(NSString *)interface {
    if (!_isSDKInit) {
        ANSDebug(@"[Analysys] [Log] The SDK is not initialized, please call %@ after initialization.", interface);
        return NO;
    } else {
        return YES;
    }
