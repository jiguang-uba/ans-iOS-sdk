...
+ (void)setPageCloseBlackList:(NSSet<NSString *> *)controllers {
    [[AnalysysSDK sharedManager] setPageCloseBlackList:controllers];
}
...
+ (NSDictionary *)getSuperProperties {
    return [[AnalysysSDK sharedManager] getSuperPropertiesValueSync];
}

+ (id)getSuperProperty:(NSString *)superPropertyName {
    return [[AnalysysSDK sharedManager] getSuperPropertySync:superPropertyName];
}
...
+ (void)setAnalysysAgentHybrid:(WKWebViewConfiguration *)config scriptMessageHandler:(id)handler {
    [self setAnalysysAgentHybrid:config];
}
...
+ (void)setAnalysysAgentHybrid:(WKWebViewConfiguration *)config{
    [[AnalysysSDK sharedManager] setAnalysysAgentHybrid:config];
    [[AnalysysSDK sharedManager] setWebViewVisualConfig:config];
}
...
+ (AnalysysSSManagerAAABBB *)getSSManagerInstance {
